...
class IntegrationBase
{
  public:
    ...
    /*
    构造函数（IntegrationBase）:VINS-Mono要求预积分中的参数必须进行初始化，因此移除了默认构造函数，仅允许使用如下所示的唯一构造函数：
    该构造函数接收第k帧时刻对应的加速度、角速度、加速度计零偏和陀螺仪零偏，并完成各成员变量的初始化，初始化中的各成员变量含义上一节已说明。
    */
    ...
    加速度计量测值acc和陀螺仪量测值gyr，并基于量测值对IMU预积分项进行一步递推，*/
    ...
    /*状态重传播函数（reprogate）
    该函数在后端完成优化，给出加速度计和陀螺仪零偏新的最优估计值后，基于新的估计值对已有的
    IMU预积分进行重递推，函数入口参数为优化后的零偏值_linearized_ba和_linearized_bg。*/
    ...
    /*
    中值积分函数（midPointIntegration）:该函数用于求解IMU的预积分状态递推和更新对应的Jacobian矩阵.
    函数接口参数分别为：
    ...
    和论文中推导采用欧拉积分法不同，代码中实际采用的是中值积分法，且认为在整个[k, k+1]过程中，加速度计和陀螺仪零偏保持不变。
    */
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        //Jacobian矩阵更新
        //该部分代码内容与前文的理论推倒基本一致，但噪声传播矩阵V的前四行与理论推导正负相反，
        if(update_jacobian)
        {
    ...
    /*
    状态传播函数（propagate）
    该函数的内核为上述中值积分函数，主要用于实现IMU预积分的递推，
    函数的入口参数分别为IMU预积分的时间间隔_dt和当前IMU量测_acc_1、_gyr_1。
    propagate函数首先调用midPointIntegration计算IMU预积分的一步递推，
    随后对IMU预积分初值进行重置（注意每次递推需要对四元数进行归一化），用于下一步递推
    */
    ...
    函数返回值为k和k+1之间IMU预积分状态与非线性优化状态之间的残差，
    用于构建后端的非线性优化误差函数（理想状态下残差应该为零，实际情况下残差应该逐渐收敛于一个小值附近）
    */
    ...
    Eigen::Matrix<double, 15, 15> step_jacobian;//系统噪声矩阵
